package group10.client.constants;

/**
 * Holds UI messages to inform user about an event
 *
 * @author Alperen Caykus, Mustafa Ozan Alpay
 */
public class UiInfoConstants {
    /**
     * Message for invalid login credentials.
     */
    public final static String LOGIN_ERROR_MESSAGE = "Wrong credentials!";
    /**
     * Message for empty field that should have been filled.
     */
    public final static String EMPTY_FIELD_ERROR_MESSAGE = "Please fill all the fields";
    /**
     * Message for user not found.
     */
    public final static String USER_NOT_FOUND = "User not found.";
    /**
     * Message for reset code sending successful.
     */
    public final static String EMAIL_SENT_SUCCESS = "Password Reset code has been successfully sent.";
    /**
     * Success message for successful password update.
     */
    public final static String PASSWORD_RESET_SUCCESS = "Password successfully updated.";
    /**
     * Error message for invalid password reset code.
     */
    public final static String CODE_DONT_MATCH_MESSAGE = "Username and code do not match.";
    /**
     * Empty string
     */
    public final static String EMPTY_STRING = "";
    /**
     * Red color to make error texts red
     */
    public final static String RED_COLOR = "#ff0000";
    /**
     * Green color to make success texts green
     */
    public final static String GREEN_COLOR = "#00ab36";
    /**
     * Button text for change password
     */
    public final static String BUTTON_RESET_TEXT = "Change Password";
    /**
     * Button text for send email
     */
    private final static String BUTTON_EMAIL_TEXT = "Send Email";
}
